...
class Session(Base):
    ...
    # генерируется автоматически через secrets.token_hex(32)
    id = Column(
        String(64),
        primary_key=True,
    ...
    # (при удалении пользователя удаляются и его сессии)
    user_id = Column(
        Integer,
    ...
    # Токен сессии — отправляется клиенту и используется для
    # аутентификации в последующих запросах
    token = Column(String(128), unique=True, nullable=False, index=True)

    # Время истечения сессии — после этого времени токен недействителен
    expires_at = Column(DateTime(timezone=True), nullable=False)

    # Время создания сессии
    created_at = Column(DateTime(timezone=True), server_default=func.now())

    # Связь "многие к одному" — объект сессии будет иметь атрибут .user
    # backref добавляет .sessions к объекту User
