...
# Получаем строку подключения из .env, если нет — используем значение по умолчанию
# Формат: postgresql://пользователь:пароль@хост:порт/имя_базы
DATABASE_URL = os.getenv(
    "DATABASE_URL",                                     # Ключ в .env файле
    "postgresql://username:password@localhost/spellcheck_db"  # Значение по умолчанию (замените на своё!)
)
...
# Создаём фабрику сессий — объект, который будет создавать сессии для работы с БД
SessionLocal = sessionmaker(
    autocommit=False,                                   # Отключаем автокоммит — сами решаем, когда сохранять изменения
    autoflush=False,                                    # Отключаем авто-сброс буфера — сами управляем отправкой запросов
    bind=engine                                         # Привязываем сессию к нашему движку (подключению)
)
...
# Dependency-функция для FastAPI — автоматически открывает и закрывает сессию БД
def get_db():
    db = SessionLocal()                                 # Создаём новую сессию БД
    try:
        yield db                                        # Передаём сессию в обработчик запроса (например, в роутер)
    finally:
        db.close()                                      # Гарантированно закрываем сессию после обработки запроса